...
// 1. VERİTABANI BAĞLANTISI (Oregon Internal URL Uyumu)
...
if (!string.IsNullOrEmpty(rawUrl))
{
    try {
        ...
        finalConnString = $"Host={uri.Host};Port={dbPort};Database={uri.AbsolutePath.Trim('/')};Username={userInfo[0]};Password={userInfo[1]};SSL Mode=Require;Trust Server Certificate=true;";
    } catch { finalConnString = rawUrl; }
}
...
using (var scope = app.Services.CreateScope()) {
    try {
        ...
        Console.WriteLine(">>> DATABASE BAGLANTISI VE TABLOLAR HAZIR <<<");
    } catch (Exception ex) { Console.WriteLine($">>> DB HATASI: {ex.Message}"); }
}

// 3. CORS VE MIDDLEWARE (SignalR için kritik sıralama)

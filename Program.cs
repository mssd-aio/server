...
app.MapPost("/register", (UserDto dto) => 
    ChatData.Users.TryAdd(dto.Username, dto.Password) ? Results.Ok() : Results.BadRequest());

app.MapPost("/login", (UserDto dto) => 
    ChatData.Users.TryGetValue(dto.Username, out var p) && p == dto.Password ? Results.Ok() : Results.Unauthorized());

app.MapGet("/list-rooms", () => 
    ChatData.Rooms.Select(r => new { Name = r.Key, IsProtected = r.Value }));

app.MapGet("/", () => "🛡️ SECURE SERVER v9.1 - ONLINE");

app.MapHub<ChatHub>("/chatHub");

app.Run();

// --- KRİTİK: TÜM SINIF VE KAYIT TANIMLAMALARI PROGRAMIN EN ALTINDA OLMALIDIR ---
